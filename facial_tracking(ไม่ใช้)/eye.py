...
class Eye:
    """
    วัตถุของดวงตา คำนวณคุณสมบัติจากจุดใบหน้า

    Args:
        frame (numpy,ndarray): เฟรมอินพุต
        face_landmarks (mediapipe face landmarks object): ค่าพิกัดจุดใบหน้า
        id (list of int): ดัชนีของดวงตาในจุดใบหน้า
    """
    ...
    def _get_eye_pos(self):
        """รับตำแหน่งซ้าย ขวา บน ล่างของดวงตา"""
    ...
    def _get_gaze_ratio(self):
        """รับอัตราส่วนการจ้องมองของม่านตาเทียบกับดวงตา"""
...
def main():
    ...
    while cap.isOpened():
        ...
        frame, text = process_frame(frame, fm)
        ...
        cv2.putText(frame, f'FPS: {int(fps)}', (30,30), 0, 0.8, conf.TEXT_COLOR, 2, lineType=cv2.LINE_AA)
        cv2.putText(frame, f'{text}', (30,70), 0, 0.8, conf.WARN_COLOR, 2, lineType=cv2.LINE_AA)
