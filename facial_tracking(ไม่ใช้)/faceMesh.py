    ...
    def get_face_bbox(self):
        ...
        if self.mesh_result.multi_face_landmarks:
            for face_landmarks in self.mesh_result.multi_face_landmarks:
                ...
                # คูณค่าพิกัดด้วยขนาดของภาพ (width, height)
                return (int(x_min * conf.FRAME_W), int(y_min * conf.FRAME_H),
                        int((x_max - x_min) * conf.FRAME_W), int((y_max - y_min) * conf.FRAME_H))
        return None
